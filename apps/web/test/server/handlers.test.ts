...
} from '../../server/xcs/schema-catalog.js'
import { canonicalJson, encodeUtf8, sha256Hex } from '../../server/xcs/serialization.js'
import type { ApiRepository, SchemaProjectionEvidence } from '../../server/xcs/types.js'
...
type TestApi = ApiHandlers & { inject: ReturnType<typeof createInjector> }

function instrument(handlers: ApiHandlers): TestApi {
  return Object.assign(handlers, { inject: createInjector(handlers) })
}

function routeFor(handlers: ApiHandlers, method: 'GET' | 'POST', path: string) {
  return handlers.routes.find((route) => route.method === method && route.path === path)
}

/** The pinning routes are registered whenever a service is configured; these
 * assertions only read the route table, so the service is never called. */
function pinningServiceStub(): DemoPinningService {
  return {} as unknown as DemoPinningService
}
...
async function configuredApp(overrides: Partial<CreateApiOptions>) {
  const instance = instrument(
    createApiHandlers({
      repository: new RouteRepository(),
      resolver: { resolve: async () => new Uint8Array() },
...
describe('read API', () => {
  ...
  it('keeps deployment probes available outside the global request budget', async () => {
    ...
    for (const path of ['/health/live', '/health', '/health/ready']) {
      expect(routeFor(instance, 'GET', path)?.rateLimit).toBe(false)
    }
  ...
  it('keeps operational metrics absent unless explicitly configured', async () => {
    ...
    expect(routeFor(instance, 'GET', '/internal/metrics')).toBeUndefined()
    expect(routeFor(instance, 'GET', '/internal/metrics/prometheus')).toBeUndefined()
  })
  ...
  it('protects operational metrics, keeps them outside quotas, and hides them from OpenAPI', async () => {
    ...
    expect(routeFor(instance, 'GET', '/internal/metrics')?.rateLimit).toBe(false)
    expect(routeFor(instance, 'GET', '/internal/metrics')?.schema?.hide).toBe(true)

    instance.recordRateLimited('/v1/networks')
  ...
  it('serves OpenMetrics behind the metrics token without entering public quotas', async () => {
    ...
    expect(routeFor(instance, 'GET', '/internal/metrics/prometheus')?.rateLimit).toBe(false)
    expect(routeFor(instance, 'GET', '/internal/metrics/prometheus')?.schema?.hide).toBe(true)
  ...
  it('exposes a public DTO for durable indexer status', async () => {
    const instance = await app()
    ...
    const statusResponses = routeFor(instance, 'GET', '/v1/networks/:network/status')?.schema
      ?.response
    expect(statusResponses).toHaveProperty('200')
    expect(statusResponses).toHaveProperty('404')
    expect(response.json()).not.toHaveProperty('writerId')
    expect(response.json()).not.toHaveProperty('writerEpoch')

    expect(
      routeFor(instance, 'GET', '/v1/networks/:network/schemas/:uid')?.schema?.response,
    ).toHaveProperty('503')
    expect(
      routeFor(instance, 'GET', '/v1/networks/:network/schemas')?.schema?.response,
    ).toHaveProperty('503')
    expect(
      routeFor(
        instance,
    ...
    expect(routeFor(instance, 'POST', '/v1/verify')?.schema?.response).toHaveProperty('503')
    expect(routeFor(instance, 'POST', '/v1/verify')?.schema?.response).toHaveProperty('404')
  })
  ...
  it('exposes profile-bound signing readiness from one authoritative snapshot', async () => {
    ...
    const readinessResponses = routeFor(instance, 'GET', '/v1/networks/:network/readiness')?.schema
      ?.response
    for (const status of [200, 400, 404, 429, 503, 500]) {
      expect(readinessResponses).toHaveProperty(String(status))
    }
  ...
  it('documents closed success contracts for the developer verification flow', async () => {
    ...
    const responseSchema = (method: 'GET' | 'POST', path: string) =>
      routeFor(instance, method, path)?.schema?.response?.[200] as ContractSchema | undefined

    const exactSchemaContract = responseSchema('GET', '/v1/networks/:network/schemas/:uid')
    const schemaListContract = responseSchema('GET', '/v1/networks/:network/schemas')
    const exactCredentialContract = responseSchema(
      'GET',
      '/v1/networks/:network/credentials/:issuer/:subject/:schemaUid',
    )
    const verificationContract = responseSchema('POST', '/v1/verify')
    ...
    const fieldDescriptorContract = xcsFieldDescriptorSchema as unknown as ContractSchema
  ...
  it('declares explicit success and failure responses for every discovery endpoint', async () => {
    const instance = await app()
    for (const path of [
      ...
      const responses = routeFor(instance, 'GET', path)?.schema?.response
  ...
  it('serves an authoritative topological schema catalog from one snapshot', async () => {
    ...
    const catalogResponses = routeFor(instance, 'GET', '/v1/networks/:network/schemas/:uid/catalog')
      ?.schema?.response
    for (const status of [200, 400, 404, 429, 503, 500]) {
      expect(catalogResponses).toHaveProperty(String(status))
    }
  })
  ...
  it('looks up one exact transaction event inside the authoritative snapshot', async () => {
    ...
    const operation = routeFor(
      instance,
      'GET',
      '/v1/networks/:network/credentials/:issuer/:subject/:schemaUid/events/:transactionHash',
    )
    expect(operation?.schema?.response).toHaveProperty('200')
    expect(operation?.schema?.response).toHaveProperty('400')
    expect(operation?.schema?.response).toHaveProperty('404')
    expect(operation?.schema?.response).toHaveProperty('503')
  })
  ...
  it('carries an independent verification rate-limit budget', async () => {
    const instance = await app()
  ...
  it('counts a rate-limited verification response in the operational metrics', async () => {
    const instance = await configuredApp({
      operationalMetrics: {
        token: METRICS_TOKEN,
        repository: {
    ...
    instance.recordRateLimited('/v1/verify')
    const response = await instance.inject({
      method: 'GET',
      url: '/internal/metrics',
      headers: { authorization: `Bearer ${METRICS_TOKEN}` },
    ...
    expect(response.statusCode).toBe(200)
    expect(response.json()).toMatchObject({
      api: { rateLimitedResponses: { global: 0, verify: 1, pinning: 0 } },
    })
  })

  it('carries a stricter rate-limit budget on the demo pinning routes', async () => {
    const instance = await configuredApp({ pinningService: pinningServiceStub() })
    for (const path of ['/v1/pinning/challenges', '/v1/pinning/pins']) {
